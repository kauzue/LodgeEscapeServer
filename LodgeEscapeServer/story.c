...
item_t s_items[NUM_MAX_ITEMS_PER_PLAYER];
clue_t s_clues[NUM_MAX_CLUES_PER_PLAYER];
...
void Exit_Wait_Room(SOCKET sock, int num_players)
{
	...
	while (true) {
		if (current_player == 2) {
			send(sock, &current_player, sizeof(current_player), 0);
			break;
		}
	}
}
...
int Chapter1_Player2(SOCKET sock, int r_p_num)
{
	...
	while (explore < 1 || investigate < 1) {
		...
		case INVESTIGATE: {
			if (num_items < 1) {
				strcpy(item.name, "지갑");
				strcpy(item.information, "지갑 안에는 돈 3400원 밖에 남아있지않다.");
				memcpy(&s_items[num_items], &item, sizeof(item_t));
				++(num_items);
			}
			investigate++;
